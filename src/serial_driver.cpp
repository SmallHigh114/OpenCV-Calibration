...
namespace {
// CRC16 查找表（与 serial/ 模块 CRC 一致，初值 0xFFFF）
...
// 小端解析（与 serial/ 模块 ReceiveData 字段序一致）
...
Serial_driver::Serial_driver(const std::string& config_path): queue_(5000) {
    ...
    daemon_thread_ = std::thread([this]() {
        rx_buffer_.reserve(frame_length * 4);
        while (running_) {
            int recv_len = uart_transporter->read(tmp_buffer_, frame_length);
            if (recv_len <= 0) {
                continue;
            }
            rx_buffer_.insert(rx_buffer_.end(), tmp_buffer_, tmp_buffer_ + recv_len);

            // 按帧解析：CRC16 覆盖前 15 字节，不匹配则滑动 1 字节重新同步
            while (rx_buffer_.size() >= frame_length) {
                uint16_t crc_rx = read_u16_le(rx_buffer_.data() + frame_length - 2);
                if (crc16_calc(rx_buffer_.data(), frame_length - 2) != crc_rx) {
                ...
                auto yaw = read_i32_le(rx_buffer_.data() + 3) / 1e5;
                auto pitch = read_i32_le(rx_buffer_.data() + 7) / 1e5;
                auto roll = read_i32_le(rx_buffer_.data() + 11) / 1e5;
